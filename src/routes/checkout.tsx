...
import { CreditCard, Truck, MapPin, Loader2, Plus, Sparkles, StickyNote, AlertTriangle, User } from "lucide-react";
import { PageShell } from "@/components/page-shell";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/lib/use-auth";
import { addressesQuery, cartQuery, profileQuery, productsByIdsQuery } from "@/lib/queries";
...
import { readGuestCart, clearGuestCart, saveGuestOrder, type GuestCartItem } from "@/lib/guest-cart";
...
function CheckoutRouter() {
  ...
  if (loading) {
    return <PageShell title="إتمام الطلب"><div className="px-4 pt-8"><Loader2 className="size-6 animate-spin mx-auto text-gold" /></div></PageShell>;
  }
  return userId ? <AuthCheckout userId={userId} /> : <GuestCheckout />;
}

// ---------- AUTHENTICATED (original flow preserved) ----------
function AuthCheckout({ userId }: { userId: string }) {
  ...
  const maxPointsBySubtotal = Math.floor(subtotal / 10);
  ...
  const pointsDiscount = parsedPoints * 10;
  ...
  return (
    <PageShell title="إتمام الطلب">
      <div className="px-4 pt-4 space-y-4">
        ...
        <Section title="كلفة التوصيل" icon={<Truck className="size-4 text-gold" />}>
          <div className="text-sm text-muted-foreground">
            {shippingCost > 0 ? <>كلفة التوصيل: <span className="font-bold text-navy">{formatIQD(shippingCost)}</span></> : "توصيل مجاني"}
          </div>
        </Section>
        ...
        {pointsBalance > 0 && subtotal > 0 && (
          <Section title="استخدام نقاط الولاء" icon={<Sparkles className="size-4 text-gold" />}>
            ...
            <div className="flex gap-2">
              <input type="number" inputMode="numeric" min={0} max={maxPoints} value={pointsInput}
                onChange={(e) => setPointsInput(e.target.value)} placeholder="0"
                className="flex-1 h-11 rounded-xl border-2 border-border px-3 text-sm font-bold bg-background" />
              <button type="button" onClick={() => setPointsInput(String(maxPoints))}
                className="px-3 h-11 rounded-xl bg-gold/10 text-gold text-xs font-bold border border-gold/30">
                استخدم الحد الأقصى ({maxPoints})
              </button>
            </div>
            {parsedPoints > 0 && <div className="mt-2 text-xs text-success font-bold">خصم: {formatIQD(pointsDiscount)}</div>}
          </Section>
        )}

        <Section title="ملاحظة على الطلب" icon={<StickyNote className="size-4 text-gold" />}>
          <textarea value={orderNote} onChange={(e) => setOrderNote(e.target.value)} rows={3} maxLength={500}
      ...
      {showSplitAlert && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowSplitAlert(false)}>
          <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}
            ...
            <div className="flex items-center gap-2 mb-3">
              <div className="size-10 rounded-full bg-gold/15 grid place-items-center"><AlertTriangle className="size-5 text-gold" /></div>
            ...
            <button onClick={() => setShowSplitAlert(false)} className="mt-4 w-full h-12 rounded-2xl bg-gradient-gold text-navy font-black shadow-gold">فهمت</button>
          </div>
        </div>
      )}
    </PageShell>
...
function GuestCheckout() {
  const guestItems = useGuestCartLive();
  const { data: products = [] } = useQuery(productsByIdsQuery(guestItems.map((i) => i.product_id)));
  const adjust = useAdjustedPrice();
  const navigate = useNavigate();

  const enriched = guestItems.map((i) => ({ ...i, product: products.find((p) => p.id === i.product_id) }));
  const subtotal = enriched.reduce((s, i) => s + adjust(i.product?.price_iqd ?? 0) * i.quantity, 0);
  const shippingCost = computeShipping(enriched.map((i) => ({ quantity: i.quantity, product: i.product })) as any);
  const total = subtotal + shippingCost;

  const [form, setForm] = useState({
    full_name: "", phone: "", phone2: "", city: "بغداد", area: "", street: "",
  });
  const [payment, setPayment] = useState<"cod" | "transfer">("cod");
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      clearGuestCart();
      toast.success("تم تأكيد الطلب بنجاح");
      navigate({ to: "/order-success/$id", params: { id: row.order_id as string } });
    } catch (err: any) {
      toast.error(err?.message ? `تعذّر إتمام الطلب: ${err.message}` : "تعذّر إتمام الطلب");
      setPlacing(false);
    }
  };
    ...
    return (
      <PageShell title="إتمام الطلب">
        <div className="px-6 py-20 text-center">
          <p className="text-sm text-muted-foreground mb-6">السلة فارغة</p>
  ...
  return (
    <PageShell title="إتمام الطلب">
      <div className="px-4 pt-4 space-y-4">
        <div className="rounded-2xl border border-gold/30 bg-gold/5 p-3 flex items-start gap-2">
          <User className="size-4 text-gold shrink-0 mt-0.5" />
          <div className="text-[12px] leading-5 text-foreground">
            تطلب كضيف — لا حاجة للتسجيل.
            <Link to="/auth" className="text-gold font-bold mx-1">أو سجّل دخول</Link>
            للحصول على النقاط، المفضلة، وحفظ عناوينك.
          </div>
        </div>

        <Section title="معلومات التوصيل" icon={<MapPin className="size-4 text-gold" />}>
          <div className="space-y-2">
            <Input placeholder="الاسم الكامل *" value={form.full_name} onChange={(v) => upd("full_name", v)} />
            <Input placeholder="رقم الهاتف *" value={form.phone} onChange={(v) => upd("phone", v)} type="tel" />
            <Input placeholder="رقم هاتف احتياطي (اختياري)" value={form.phone2} onChange={(v) => upd("phone2", v)} type="tel" />
            <Input placeholder="المحافظة *" value={form.city} onChange={(v) => upd("city", v)} />
            <Input placeholder="المنطقة *" value={form.area} onChange={(v) => upd("area", v)} />
            <Input placeholder="أقرب نقطة دالة (اختياري)" value={form.street} onChange={(v) => upd("street", v)} />
          </div>
        </Section>
        ...
        <div className="bg-card rounded-2xl border border-border p-4 shadow-card">
          <div className="text-xs font-bold text-gold mb-3">ملخص الطلب</div>
          <div className="flex justify-between text-sm py-1"><span className="text-muted-foreground">{guestItems.length} منتج</span><span>{formatIQD(subtotal)}</span></div>
          <div className="flex justify-between text-sm py-1"><span className="text-muted-foreground">التوصيل</span><span>{formatIQD(shippingCost)}</span></div>
          <div className="border-t border-border mt-2 pt-3 flex justify-between items-baseline">
            <span className="font-bold">الإجمالي</span>
            <span className="text-2xl font-black text-navy">{formatIQD(total)}</span>
        ...
        <button onClick={placeOrder} disabled={placing || !canSubmit}
          className="w-full h-14 rounded-2xl bg-gradient-gold text-navy font-black shadow-gold flex items-center justify-center gap-2 disabled:opacity-50">
          {placing && <Loader2 className="size-5 animate-spin" />}
          تأكيد الطلب
        </button>
