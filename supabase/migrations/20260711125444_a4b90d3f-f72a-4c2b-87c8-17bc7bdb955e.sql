-- 1) Allow guest orders (user_id nullable)
ALTER TABLE public.orders ALTER COLUMN user_id DROP NOT NULL;

-- 2) Add guest_token to allow retrieving/tracking a guest order without a login
...
-- 3) place_guest_order RPC: accepts items JSON + address; no auth required
...
BEGIN
  ...
  -- Shipping (same grouping logic as place_order)
...
GRANT EXECUTE ON FUNCTION public.place_guest_order(jsonb, jsonb, text, text) TO anon, authenticated;

-- 4) Allow guest to read their order by (order_number + guest_token) via RPC
  ...
  SELECT jsonb_build_object(
    ...
    'items', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'id', oi.id, 'name_ar', oi.name_ar, 'oem_number', oi.oem_number,
        'image_url', oi.image_url, 'unit_price_iqd', oi.unit_price_iqd,
        'quantity', oi.quantity, 'side', oi.side, 'note', oi.note
      )), '[]'::jsonb)
      FROM public.order_items oi WHERE oi.order_id = o.id
    )
  ) INTO v_result
  FROM public.orders o
  WHERE o.order_number = p_order_number AND o.guest_token = p_guest_token AND o.is_guest = true;
