...
ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS merge_delivery boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS delivery_group text;

CREATE INDEX IF NOT EXISTS idx_products_delivery_group ON public.products(delivery_group) WHERE delivery_group IS NOT NULL;
...
BEGIN
  ...
  -- Delivery calculation: group by delivery_group when merge_delivery=true, else per-product independent.
  -- Charge MAX(shipping_iqd) per group.
