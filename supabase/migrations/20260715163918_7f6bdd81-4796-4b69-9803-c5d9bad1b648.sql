--------------------------------------------------------------------------------
-- Audit Step 2: block customer order tampering + fix stock oversell race
--------------------------------------------------------------------------------

-- PART 1: customers must not delete or late-cancel orders --------------------
DROP POLICY IF EXISTS "Users can delete own orders" ON public.orders;
DROP POLICY IF EXISTS "Users can cancel own pending orders" ON public.orders;

CREATE POLICY "Users can cancel own early orders"
  ON public.orders
  FOR UPDATE
  TO authenticated
...
-- PART 2: stop stock overselling --------------------------------------------
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS stock_qty_non_negative;
ALTER TABLE public.products ADD CONSTRAINT stock_qty_non_negative CHECK (stock_qty >= 0) NOT VALID;
ALTER TABLE public.products VALIDATE CONSTRAINT stock_qty_non_negative;
